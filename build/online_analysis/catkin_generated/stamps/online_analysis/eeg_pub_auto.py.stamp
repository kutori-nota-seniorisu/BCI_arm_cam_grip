...
def talker():
	...
	rawdata = scio.loadmat('/home/BCI_arm_cam_grip/eegdata_del3s_v7.mat')
	eegdata = np.array(rawdata['eegdata'])
	...
	while not rospy.is_shutdown():
		packet = eegdata[:,
						 packet_i * packetSize: (packet_i + 1) * packetSize,
						 target_i,
						 exper_i].T
		packet_pub.data = packet.reshape(35 * 512)
		rospy.loginfo(
			"I publish: %i exper, %i target, %i packet",
			exper_i + 1,
			target_i + 1,
			packet_i + 1)
		pub.publish(packet_pub)
		packet_i = packet_i + 1
		if packet_i % packetNum == 0:
			packet_i = 0
			target_i = target_i + 1
